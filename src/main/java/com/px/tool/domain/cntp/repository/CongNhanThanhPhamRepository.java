...
public interface CongNhanThanhPhamRepository extends JpaRepository<CongNhanThanhPham, Long> {
    ...
    @Query(value = "SELECT c.* , n.* FROM cong_nhan_thanh_pham AS c INNER JOIN noi_dung_thuc_hien as n ON n.tp_id = c.tp_id WHERE n.nghiem_thu = ?1 AND c.tpkcs_xac_nhan <> 1 AND (c.step IS NULL OR c.step <> 1 OR c.step = 0) ", nativeQuery = true)
