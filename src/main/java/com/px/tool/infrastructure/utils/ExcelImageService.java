...
public class ExcelImageService {
    ...
    /**
     * Add image on sheet ${sheet} into shell ${cell} with imageData ${imageData}, default image size: h=30, w=45
     */
    public void addImageToSheet(String cellNumber, Sheet sheet, byte[] imageData) throws IOException, IllegalArgumentException {
        ...
        this.addImageToSheet(cellRef.getCol(), cellRef.getRow(), sheet, imageData, Workbook.PICTURE_TYPE_JPEG, DEFAULT_WIDTH, DEFAULT_HEIGHT, EXPAND_ROW_AND_COLUMN);
    }

    public void addImageToSheet(String cellNumber, Sheet sheet, byte[] imageData, int imageFile, double reqImageWidthMM, double reqImageHeightMM,
                                int resizeBehaviour) throws IOException, IllegalArgumentException {
        // Convert the String into column and row indices then chain the
        // call to the overridden addImageToSheet() method.
        CellReference cellRef = new CellReference(cellNumber);
        this.addImageToSheet(cellRef.getCol(), cellRef.getRow(), sheet, imageData, imageFile, reqImageWidthMM, reqImageHeightMM, resizeBehaviour);
    }

    public void addImageToSheet(int colNumber, int rowNumber, Sheet sheet, byte[] imageData, int imageType, double reqImageWidthMM, double reqImageHeightMM,
        ...
        if ((resizeBehaviour != ExcelImageService.EXPAND_COLUMN) &&
                (resizeBehaviour != ExcelImageService.EXPAND_ROW) &&
                (resizeBehaviour != ExcelImageService.EXPAND_ROW_AND_COLUMN) &&
                (resizeBehaviour != ExcelImageService.OVERLAY_ROW_AND_COLUMN)) {
        ...
        int index = sheet.getWorkbook().addPicture(imageData, imageType);
        ...
        if (colWidthMM < reqImageWidthMM) {
            ...
            if ((resizeBehaviour == ExcelImageService.EXPAND_COLUMN) ||
                ...
                if (sheet instanceof HSSFSheet) {
                    ...
                    pictureWidthCoordinates = (int) reqImageWidthMM * ExcelImageService.EMU_PER_MM;
                }
            ...
            else if ((resizeBehaviour == ExcelImageService.OVERLAY_ROW_AND_COLUMN) ||
                    (resizeBehaviour == ExcelImageService.EXPAND_ROW)) {
    ...
    private ClientAnchorDetail fitImageToRows(Sheet sheet, int rowNumber,
        ...
        if (rowHeightMM < reqImageHeightMM) {
            if ((resizeBehaviour == ExcelImageService.EXPAND_ROW) ||
                    (resizeBehaviour == ExcelImageService.EXPAND_ROW_AND_COLUMN)) {
            ...
            // the image out ver one or more rows.
                    ...
                    (resizeBehaviour == ExcelImageService.EXPAND_COLUMN)) {
    ...
    private ClientAnchorDetail calculateColumnLocation(Sheet sheet,
        ...
        if ((int) totalWidthMM == (int) reqImageWidthMM) {
            ...
            if (sheet instanceof HSSFSheet) {
                ...
                anchorDetail = new ClientAnchorDetail(startingColumn,
                        toColumn, (int) reqImageWidthMM * ExcelImageService.EMU_PER_MM);
            }
        }
        ...
        // for the ClientAnchorDetail object.
            ...
            if (sheet instanceof HSSFSheet) {
                ...
                inset = (int) overlapMM * ExcelImageService.EMU_PER_MM;
            }
    ...
    private ClientAnchorDetail calculateRowLocation(Sheet sheet,
        ...
        if ((int) totalRowHeightMM == (int) reqImageHeightMM) {
            if (sheet instanceof HSSFSheet) {
                ...
                clientAnchorDetail = new ClientAnchorDetail(startingRow, toRow,
                        (int) reqImageHeightMM * ExcelImageService.EMU_PER_MM);
            }
        } else {
